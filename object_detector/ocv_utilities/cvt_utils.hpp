...
namespace cvt {
    ...
    enum class Gumby { gumby, pokey, minga, goo, prickle };
    ...
    // The last 3 classes are intended to prevent Gumby and Pokey false positives.  We may want to remove them
    // from this list so we only report a Gumbey or a Pokey.
    static const std::vector<std::string> GUMBY_CLASS_LABELS{"Gumby", "Pokey", "Minga", "Goo", "Prickle"};
    ...
    static void drawPred(int classId, float conf, int left, int top, int right, int bottom, const cv::Mat& frame, float distance)
    {
        ...
        std::string label = cv::format("%.2f, %.2f", conf, distance);
        if (!classes.empty())
        {
        ...
        rectangle(frame, cv::Point(left, top - topShift), cv::Point(left + leftShift, top + baseLine), cv::Scalar(255, 255, 255), cv::FILLED);
        putText(frame, label, cv::Point(left, top), cv::FONT_HERSHEY_SIMPLEX, 0.75, cv::Scalar(0,0,0),1);
    }

    // Remove the bounding boxes with low confidence using non-maxima suppression
    static std::vector<FoundObject> postprocess(const cv::Mat& frame, const std::vector<cv::Mat>& outs,
                            bool drawBoxes = false, const cv::Mat& depth = cv::Mat(), float depth_scale=0.0f)
    {
        ...
        cv::dnn::NMSBoxes(boxes, confidences, cvt::CONF_THRESHOLD, cvt::NMS_THRESHOLD, indices);
        ...
        for (int idx: indices) {
            ...
            if (drawBoxes) {
                drawPred(id, conf, box.x, box.y,
                         box.x + box.width, box.y + box.height, frame, f);
            }
        }
        return objs;
    }
